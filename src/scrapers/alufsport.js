...
class AlufSportScraper extends BaseScraper {
    constructor(searchInput) {
        super(searchInput, 'Aluf Sport');
        this.baseUrl = 'https://www.alufsport.co.il/';
        this.searchUrl = (query) => `https://www.alufsport.co.il/?s=${encodeURIComponent(query)}&post_type=product`;
    }
    ...
    async scrapePage(page, url, targetModels, targetSizes) {
        ...
        // Aluf Sport might be heavy or protected, wait for network idle
        ...
        const products = await page.evaluate(() => {
            ...
            elements.forEach(el => {
                try {
                    ...
                    // Aluf might use data-lazy-src or similar
                    const image = imgEl ? (imgEl.getAttribute('data-src') || imgEl.getAttribute('data-lazy-src') || imgEl.src) : '';
                    ...
                    if (title && price && !isNaN(price) && !isOutOfStock) {
                        items.push({ title, price, link, image, store: 'Aluf Sport' });
                    }
                } catch (err) {
                    // ignore
                }

...
class Shoes2uScraper extends BaseScraper {
    constructor(searchInput) {
        super(searchInput, 'Shoes2u');
        this.baseUrl = 'https://www.shoes2u.co.il/';
        // Standard WP search since /search/?q=... failed
        this.searchUrl = (query) => `https://www.shoes2u.co.il/?s=${encodeURIComponent(query)}&post_type=product`;
    }
            ...
            elements.forEach(el => {
                try {
                    ...
                    if (title && price && !isNaN(price) && !isOutOfStock) {
                        items.push({ title, price, link, image, store: 'Shoes2u' });
                    }
                } catch (err) {
                    // ignore
                }

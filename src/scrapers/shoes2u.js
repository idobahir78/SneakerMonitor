...
class Shoes2uScraper extends BaseScraper {
    constructor(searchTerm) {
        ...
        if (!query) throw new Error("Search term is required for Shoes2uScraper");
        super('Shoes2u', `https://www.shoes2u.co.il/?s=${encodeURIComponent(query)}&post_type=product`);
    }
            ...
            elements.forEach(el => {
                ...
                if (titleEl && linkEl) {
                    ...
                    if (title && price && !isOutOfStock) {
                        items.push({ title, price, link, store: 'Shoes2u', sizes: [] });
                    }
                }
            });
            return items;

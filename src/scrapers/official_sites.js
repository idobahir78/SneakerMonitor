...
const BRAND_CONFIG = {
    'Nike': { url: 'https://www.nike.com/w/mens-shoes-nik1zy7ok', sel: '.product-card__title' },
    'Puma': { url: 'https://us.puma.com/us/en/men/shoes', sel: '[data-test-id="product-list-item-title"], .tw-xwxa0o' },
    'Adidas': { url: 'https://www.adidas.com/us/en/men-shoes', sel: '.gl-product-card__title' },
    'New Balance': { url: 'https://www.newbalance.com/men/shoes/', sel: '.product-name' },
    'Asics': { url: 'https://www.asics.com/us/en-us/mens-shoes/c/aa10000000/', sel: '.product-name' }
};
...
async function scrapeOfficialSites() {
    ...
    try {
        ...
        for (const [brand, config] of Object.entries(BRAND_CONFIG)) {
            ...
            try {
                const res = await page.goto(config.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
                if (res && res.status() < 400) {
                    ...
                    console.log(`✅ ${brand} Scraped: Found ${results[brand].size} models directly from store.`);
                } else {
                    console.log(`❌ ${brand} Blocked or failed (Status ${res ? res.status() : 'Unknown'})`);
                }
            } catch (e) {
                console.log(`⚠️ ${brand} timeout or navigation error: ${e.message}`);
            } finally {
                await page.close();
            }
        }

...
class LimeShoesScraper extends BaseScraper {
    constructor(searchInput) {
        super(searchInput, 'Lime Shoes');
        this.baseUrl = 'https://limeshoes.co.il/';
        this.searchUrl = (query) => `https://limeshoes.co.il/?s=${encodeURIComponent(query)}&post_type=product`;
    }
            ...
            elements.forEach(el => {
                try {
                    ...
                    if (title && price && !isNaN(price) && !isOutOfStock) {
                        items.push({ title, price, link, image, store: 'Lime Shoes' });
                    }
                } catch (err) {
                    // ignore
                }

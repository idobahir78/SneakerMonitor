...
class MayersScraper extends BaseScraper {
    constructor(searchTerm) {
        const query = searchTerm;
        if (!query) throw new Error("Search term is required for MayersScraper");
        super('Mayers', `https://mayers.co.il/?s=${encodeURIComponent(query)}&post_type=product`);
    }
        ...
        return await page.evaluate(() => {
            ...
            if (document.body && document.body.classList.contains('single-product')) {
                ...
                if (titleEl) {
                    ...
                    items.push({ store: 'Mayers', title, price, link, sizes: [] });
                    return items;
                }
            }
            ...
            elements.forEach(el => {
                ...
                if (titleEl && linkEl) {
                    ...
                    if (title && price && !isOutOfStock) {
                        items.push({ title, price, link, store: 'Mayers', sizes: [] });
                    }
                }
            });
            return items;

...
class MayersScraper extends BaseScraper {
    constructor(searchInput) {
        super(searchInput, 'Mayers');
        this.baseUrl = 'https://mayers.co.il/';
        this.searchUrl = (query) => `https://mayers.co.il/?s=${encodeURIComponent(query)}&post_type=product`;
    }
            ...
            elements.forEach(el => {
                try {
                    ...
                    if (title && price && !isNaN(price) && !isOutOfStock) {
                        items.push({ title, price, link, image, store: 'Mayers' });
                    }
                } catch (err) {
                    // ignore
                }

...
class MasterSportScraper extends BaseScraper {
    constructor(searchInput) {
        super(searchInput, 'Master Sport');
        this.baseUrl = 'https://mastersport.co.il/';
        this.searchUrl = (query) => `https://mastersport.co.il/?s=${encodeURIComponent(query)}&post_type=product`;
    }
            ...
            elements.forEach(el => {
                try {
                    ...
                    if (title && price && !isNaN(price) && !isOutOfStock) {
                        items.push({ title, price, link, image, store: 'Master Sport' });
                    }
                } catch (err) {
                    // ignore
                }
